...
wr = csv.writer(f)
wr.writerow(['지점명', '주소', '테마수','전화번호','카페이미지url'])
...
for x in range(1,14):
    ...
    img_path = driver.find_element(By.XPATH,'/html/body/div[3]/div/div['+str(x)+']/div/a').get_attribute('href')
    print(x)
    data = ['넥스트에디션 '+ room, loc , theme_num , call,img_path]
    wr.writerow(data)
    print(data)
